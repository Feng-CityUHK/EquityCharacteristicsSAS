...
outfile='/scratch/cityuhk/xinhe/eqchars/abr_rdqabr.csv' dbms=csv replace; run;

proc sort data=rdqabr nodupkey; by permno date; run;

proc export data = rdqabr(where=(year(datadate)>=2017))
outfile='/scratch/cityuhk/xinhe/eqchars/abr_rdqabr_nodup.csv' dbms=csv replace; run;
